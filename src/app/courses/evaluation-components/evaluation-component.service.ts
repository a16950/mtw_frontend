...
export class EvaluationComponentService {
  ...
  constructor(
    ...
    addEvaluation(course: Course, evaluation:EvaluationComponent): Observable<EvaluationComponent> {
      return this.http.post<EvaluationComponent>(this.buildEvaluationUrl(course), evaluation, this.httpOptions).pipe(
        catchError(this.serviceHelpers.handleError<EvaluationComponent>('addEvaluation'))
      );
    }

    deleteEvaluation(id: number, course:Course): Observable<EvaluationComponent> {
      ...
      return this.http.delete<EvaluationComponent>(url, this.httpOptions).pipe(
        ...
        catchError(this.serviceHelpers.handleError<EvaluationComponent>('deleteEvaluationComponent'))
      );
    }

    updateEvaluation(evaluation: EvaluationComponent, course:Course): Observable<any> {
      const ur = this.buildEvaluationUrl(course);
    ...
    getEvaluation(id: number, course:Course): Observable<EvaluationComponent> {
      ...
      return this.http.get<EvaluationComponent>(url).pipe(
        catchError(this.serviceHelpers.handleError<EvaluationComponent>(`getEvaluation id=${id}`))
      );
    }
